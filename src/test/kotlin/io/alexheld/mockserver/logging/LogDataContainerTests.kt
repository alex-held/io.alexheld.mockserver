...
class LogDataContainerTests : WithTestResources {
    ...
    companion object {
        ...
        private fun <T : DataContainerData> createSubject(type: LogMessageType, container: T): IdentifiableLog<T> =
            IdentifiableLog.generateNew(ApiCategory.Log,
        ...
        fun logDataContainerData(): Stream<Arguments> = Stream.of(
            ...
            Arguments.of(
                "exception.yaml",
                createSubject(LogMessageType.Exception,
                    ExceptionData("an error message"))
            ),
            Arguments.of(
                "log-deleted.yaml",
                createSubject(LogMessageType.Operation,
            ...
            Arguments.of(
                "request-received.yaml",
                createSubject(LogMessageType.Request_Received,
                    RequestReceivedData(Request(method = "POST")))
            ),
            Arguments.of(
                "action.yaml",
                createSubject(LogMessageType.Action_Response,
